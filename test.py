...
import csv
import pdfplumber
import pandas as pd  # ✅ Using pandas for a structured table output
...
output_csv = r"D:\Code\Output\invoices_data.csv"
...
def extract_invoice_details(text):
    ...
    total_amount = total_amount_match.group(1).strip() if total_amount_match else "0.00"
...
# ✅ Save as a properly formatted CSV using Pandas
df = pd.DataFrame(data_list)

# ✅ Ensure numeric values are saved correctly for table formatting
df["Total Amount Due"] = df["Total Amount Due"].astype(float)

# ✅ Save CSV with proper table format
df.to_csv(output_csv, index=False)

print(f"✅ Data successfully extracted and saved to {output_csv} in proper table format.")
